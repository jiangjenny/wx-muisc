<template>
  <div class="page-container">
    ...
    <div class="banner">
      ...
      >
        <block v-for="item in banner" :key="item.targetId">
    ...
    <div class="Entrance">
      ...
      <div class="EntranceIcon" @click="moreSongsList">
      ...
      <div class="EntranceIcon" @click="ranking">
    ...
    <div class="songList">
      ...
      <div class="Single">
        <div v-for="(item,index) in Single" :key="item.id" class="SingWidth">
          <div class="SingleImg" @click="musicUrl(item.id)">
            <img :src="item.song.album.picUrl" alt="">
             <div class="SingText">
                <p class="Singname">{{item.name}}</p>
                <div class="people">
                  <p v-for="(itempeo,indexpeo) in item.song.album.artists" :key="itempeo.id">
                      {{itempeo.name}} &nbsp;
                  </p>
                </div>
              </div>
...
export default {
  ...
  created () {
    ...
    // banner
    fly.get("http://127.0.0.1:4000/banner", {})
    .then(d => {
    ...
    .catch(err => {
     ...
     // 今日单曲
    fly.get("http://127.0.0.1:4000/personalized/newsong", {})
    .then(d => {
    ...
    // 今日电台
    fly.get("http://127.0.0.1:4000/personalized/djprogram", {})
    .then(d => {
  ...
  methods: {
    ...
    moreSongsList(){
      const url= "../MoreSongList/main";
      wx.navigateTo({url});
    },
    ranking(){
      const url= "../ranking/main";
      wx.navigateTo({url});
    },
    songItem(e){
    ...
    JoinTheList(e){
      ...
      console.log(playing)
      for(let i = 0; i<palying.length;i++){
        const items = playing[i].id;
        chongfu.push(items)
      }
      for(let i = 0; i<chongfu.length;i++){
        const item = chongfu[index];
        if(item == e){
          var ischongfu = true
        }
      }
