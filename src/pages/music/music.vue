<template>
  <div class="page-container">
    <div class="author">
      <div class="author name" v-for="item in musicDetailsName" :key="item.id">
        {{item.name}}
      </div>
    </div>
    <!-- 旋转图片 -->
    ...
    <swiper
      ...
      class="musicComment"
    >
      <block v-for="item in musicComment" :key="item.id">
        <swiper-item class="musicItem">
          <img :src="item.user.avatarUrl" />:&nbsp;&nbsp;{{item.content}}
        </swiper-item>
      </block>
    </swiper>
    <!-- 下载收藏 -->
    <div class="musicMini">
      <i v-bind:style="[dianzan.zhuangtai?dianzan.zhen:dianzan.jia, errorClass]" @click="dianzanClick"></i>
...
export default {
  ...
  onLaunch(){
    let backgroundAudioManager = wx.getBackgroundAudioManager(); //获取全局唯一的背景音频管理器
  },
  mounted(){
    // 监听音乐播放
    ...
    wx.getBackgroundAudioManager().onStop(()=>{
        this.rotate = false;
        this.zanting.zhuangtai = false;
        wx.setStorageSync("playing",[]);  //播放记录至为空
        wx.setStorageSync("status", false);  //音乐暂停初始化为false
        wx.setStorageSync("musicId", '');  //musicId初始化为空
  ...
  methods:{
    getData(){
       //-----------------------  第一 设置页面进来时，将歌曲存入到本地播放列表 ---------------------
      // 1 目的是判断当前点击的歌曲是否已经加入到播放列表
      let playing = wx.getStorageSync('playing'); //获取本地存储播放记录
      let chongfu = [];
      for(let i=0;i<playing.length;i++){
        const item = playing[i];
        if(item.id==this.id){  //如果两id相等,说明正在播放
        console.log(item.id+"------"+this.id);
          item.status = true;  //设置背景音乐的状态
          var sameitemID = true  //后面主要是表示当两ID相同时，设置状态
        }else{
          item.status = false;  //如果当前的id和item.id不同，说明歌曲未播放，
        }
        chongfu.push(item.id) //将已经存在的歌曲ID加入数组，过滤掉id为undefined的
        // let control = [];
        if(item.status==true){  //当前播放歌曲与本地存储背景音频播放歌曲一致
          // control.push(item);
          // console.log(control);
          wx.setStorageSync("control",item.status)
        }
      }
      // 2 循环遍历chongfu数组,得到每一项
      for(let i = 0; i<chongfu.length;i++){
        let items = chongfu[i]
      }
      // 3 判断如果不相同的id，表明列表还没有歌曲，设置其状态加入到playing中
      if(!sameitemID){ 
        let status = {  
          id: this.id,
          status: false
      ...
      var postsCollected = wx.getStorageSync('posts_collected'); //加载完毕先获取postsCollected
      if(postsCollected){  //如果存在 说明不是首次进入
      ...
      // ---------------------   第三  页面请求 -----------------------
      // 判断是第一次添加歌曲还是第二次
      if(this.id==musicId){  //musicId原本本地中没有，当两者相等，说明不是第一次进入
        fly.get("http://127.0.0.1:4000/song/url?id="+this.id,{})
        .then(d=>{
          this.musicUrlData = d.data.data[0].url;
          wx.getStorageSync('playing');
          for(let i=0;i<playing.length;i++){
            let playingStatus = playing[i].status
            if(playingStatus==true && this.status == true){  //说明背景音乐正在播放 此时转台和按钮改变状态
              this.rotate = true;
              this.zanting.zhuangtai = true;
            }
          }
          backgroundAudioManager.onEnded(()=>{ //监听音乐自然播放结束
            this.rotate = false;
            this.zanting.zhuangtai = false;
        ...
        fly.get("http://127.0.0.1:4000/song/url?id="+this.id,{})  //请求地址有误将music改成song
        .then(d=>{
          this.musicUrlData = d.data.data[0].url;
          this.rotate = true;
          this.zanting.zhuangtai = true;
          // 第一次进来将播放状态改为true 自动播放 控制button在button事件中
          this.status = true;
           wx.setStorageSync('status', true);
          wx.playBackgroundAudio({  //src 赋予 Audio 即可播放
            dataUrl: this.musicUrlData,
          })
        })
        .catch(err=>{
          console.log(err.status,err.message);
      ...
      // 作者和封面图
      fly.get("http://127.0.0.1:4000/song/detail?ids="+this.id,{})
      .then(d=>{
        let info =  d.data.songs[0];
        this.musicDetailsName = info.ar;
        this.musicDetailsImg = info.al.picUrl;
        this.musicDetailsTitle = info.al.name;

        // 设置Audio播放的信息
        wx.playBackgroundAudio({
          title: this.musicDetailsTitle,
          coverImgUrl: this.musicDetailsImg,
          singer: this.musicDetailsName
        })

        wx.setNavigationBarTitle({
            title: this.musicDetailsTitle //页面标题为路由参数
        });
      })
      .catch(err=>{
        console.log(err.status,err.message);
      ...
      // banner
      fly.get("http://127.0.0.1:4000/comment/music?id="+this.id+"&limit=100",{})
      .then(d=>{
    ...
    // 监听点赞收藏按钮
    dianzanClick(){  
      var postsCollected = wx.getStorageSync('posts_collected');//获取本地
      var postcollected = this.id;
      postsCollected[postcollected] = !postsCollected[postcollected]; //？？首次进入postcollected默认false第二次后将状态改为true
      // 更新文章是否收藏缓存集
      // 重新写入缓存集
      wx.setStorageSync('posts_collected', postsCollected);
      this.dianzan.zhuangtai = !this.dianzan.zhuangtai //点赞状态改为true
    },
    //监听button暂停按钮  设置暂停播放
    listenerButtonPause(){
       this.status = false;
      // 如果歌曲是否第二次播放，点击播放暂停按钮进行播放
      let musicId = wx.getStorageSync("musicId");
      let twoStatus = wx.getStorageSync("status");
      if(this.id==musicId && twoStatus == false){
        this.status = true;  
        wx.setStorageSync('status', true);
        wx.playBackgroundAudio({
          dataUrl: this.musicUrlData,
        });
      }else if(this.id==musicId && twoStatus == true){
        this.status = false;  
        wx.setStorageSync('status', false);
        wx.pauseBackgroundAudio({}); 
      }else if(twoStatus==false){  //第一次进来执行或者本地为false 
        this.status = true;  
        wx.setStorageSync('status', true);
        wx.playBackgroundAudio({
          dataUrl: this.musicUrlData,
      ...
      // 转台
      if(this.zanting.zhuangtai){  //zanting转台为true时
        this.zanting.zhuangtai = false;
      }else{
        this.zanting.zhuangtai = true;
      }
    ...
    nextSong(){
      ...
      for (let i = 0; i < playing.length; i++) {
          const items = playing[i];
          if (this.id == items.id) { //this.id是上一首 item.id是当前首
              this.i = i;
              this.id = playing[i - 1].id;
              break;
          }  
